...
import Data.List
    ( (++), map, maximum, length, transpose, head, replicate, reverse )
import Data.Maybe ( Maybe(..), fromJust, fromMaybe, isJust )
import Data.Sequence (Seq, ViewL (..), ViewR (..), (<|))
import qualified Data.Sequence as S
import Linear.V2 (V2 (..), _x, _y)
import System.IO.Unsafe ( unsafePerformIO )
import System.Random ( getStdRandom, Random(randomR), Random(..), newStdGen )
import Prelude
...
insertRandomTile = \case
  (x : xs) ->
    if checkFull (x : xs)
      then x : xs
      else do
        if checkRowFull x
          then x : insertRandomTile xs
          else do
            if checkFull xs
              then insertRandomTileInRow x : xs
              else do
                let tempNum = unsafePerformIO $ getStdRandom $ randomR (1, 10)
                if (tempNum :: Int) <= 6
                  then x : insertRandomTile xs
                  else insertRandomTileInRow x : xs
  _ -> error "Unexpected empty grid"


insertRandomTileInRow :: [Tile] -> [Tile]
insertRandomTileInRow = \case
  (x : xs) ->
    if checkTileExists x
      then x : insertRandomTileInRow xs
      else do
        if checkRowFull xs
          then makeRandomTile : xs
          else do
            let tempNum = unsafePerformIO $ getStdRandom $ randomR (1, 10)
            if (tempNum :: Int) <= 6
              then x : insertRandomTileInRow xs
...
directionStuckCheck n g = case n of
  1 -> g == transpose (leftGrid $ transpose g)
  2 -> g == transpose (map reverse $ leftGrid $ map reverse $ transpose g)
  3 -> g == map reverse (leftGrid (map reverse g))
  4 -> g == leftGrid g
  _ -> error "Unknown direction"
...
optimizeWeight g x y z i = do
  ...
  case i of
    1 ->
      if findAverage5Score g (x + 1) y z > tempScore
        then optimizeWeight g (x + 1) y z 1
        else
          if findAverage5Score g (x - 1) y z > tempScore
            then optimizeWeight g (x - 1) y z 1
            else x
    2 ->
      if findAverage5Score g x (y + 1) z > tempScore
        then optimizeWeight g x (y + 1) z 2
        else
          if findAverage5Score g x (y - 1) z > tempScore
            then optimizeWeight g x (y - 1) z 2
            else y
    3 ->
      if findAverage5Score g x y (z + 1) > tempScore
        then optimizeWeight g x y (z + 1) 3
        else
          if findAverage5Score g x y (z - 1) > tempScore
            then optimizeWeight g x y (z - 1) 3
            else z
    _ -> error "Unknown weight"
...
runRandomN g n =
  if n == 0
    then randomlyPlayBoard g
    else randomlyPlayBoard g + runRandomN g (n - 1)

largeEdgeNumberHeuristic :: Grid -> Int
largeEdgeNumberHeuristic g =
  if isJust a && tempScore == fromJust a
    || isJust b && tempScore == fromJust b
    || isJust c && tempScore == fromJust c
    || isJust d && tempScore == fromJust d
    || isJust e && tempScore == fromJust e
    || isJust f && tempScore == fromJust f
    || isJust gg && tempScore == fromJust gg
    || isJust h && tempScore == fromJust h
    || isJust i && tempScore == fromJust i
    || isJust j && tempScore == fromJust j
    || isJust k && tempScore == fromJust k
    || isJust l && tempScore == fromJust l
  then myExp tempScore else 0
  where
...
checkRowIncreasing r = case r of
  ...
  [x] -> True
  (x : xs) ->
    x <= head xs && checkRowIncreasing xs
...
checkRowDecreasing r = case r of
  ...
  [x] -> True
  (x : xs) ->
    x >= head xs && checkRowDecreasing xs
...
monteCarloPlayBoard g n (x, y, z) =
  if checkFull g && stuckCheck g
    then scoreGrid g 0
    else monteCarloPlayBoard a (n + 1) (x, y, z)
  where
    (a, _) = oneBestMove g (x, y, z)
...
randomlyPlayTilWin :: Int -> Grid -> Int
randomlyPlayTilWin n g
  | randomlyPlayBoard g == 2048 = n
  | n == 10000 = n
  | otherwise = randomlyPlayTilWin (n + 1) g
...
oneMove g = do
  let randomDirection = unsafePerformIO $ getStdRandom $ randomR (1, 4)
  if (randomDirection :: Int) == 1
    then insertRandomTile $ transpose $ leftGrid $ transpose g
    else
      if (randomDirection :: Int) == 2
        then insertRandomTile $ transpose $ map reverse $ leftGrid $ map reverse $ transpose g
        else
          if (randomDirection :: Int) == 3
            then insertRandomTile $ map reverse $ leftGrid (map reverse g)
            else
              if (randomDirection :: Int) == 4
                then insertRandomTile $ leftGrid g
                else g
...
generateMoveList g n (x, y, z) list =
  if checkFull g && stuckCheck g
    then (scoreGrid g 0, list)
    else generateMoveList a (n + 1) (x, y, z) (b : list)
  where
    (a, b) = oneBestMove g (x, y, z)

oneBestMove2 :: Grid -> (Int, Int, Int) -> Grid
oneBestMove2 g (x, y, z) =
  case findBestMove g (x, y, z) of
    1 -> insertRandomTile $ transpose $ leftGrid $ transpose g
    2 -> insertRandomTile $ transpose $ map reverse $ leftGrid $ map reverse $ transpose g
    3 -> insertRandomTile $ map reverse $ leftGrid (map reverse g)
